...
$(function(){
	...
	tree.tree({
		...
		onClick : function(node){
			//执行树的isLeaf方法，作用是判断节点是否是叶子节点，如果是叶子节点则执行以下逻辑
			if(tree.tree("isLeaf",node.target)){
				//执行reload的reload方法，作用是重载datagrid数据。
				//其实就是重启发起了一次请求，从后台获取内容数据，请求的参数是分类id
...
var contentListToolbar = [{
    ...
    handler:function(){
    	...
    	//if(node):判断node不为空或者不是未定义
    	//$("#contentCategoryTree").tree("isLeaf",node.target):判断节点是否是叶子节点
    	//如果node为空，或者是未定义，或者不是叶子节点，执行以下逻辑
